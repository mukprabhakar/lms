...
const Purpose = () => {
  ...
  const handleSubmit = (e) => {
    ...
    if (isEditMode) {
      setPurposes(purposes.map(purpose => 
        purpose.id === currentId ? { ...formData, id: currentId } : purpose
      ));
    } else {
      const newPurpose = {
        ...formData,
        id: purposes.length > 0 ? Math.max(...purposes.map(p => p.id)) + 1 : 1
      };
  ...
  const handleEdit = (purpose) => {
    ...
    setIsEditMode(true);
    setCurrentId(purpose.id);
    setIsFormOpen(true);
  };

  const handleDelete = (id) => {
    if (window.confirm('Are you sure you want to delete this purpose?')) {
      setPurposes(purposes.filter(purpose => purpose.id !== id));
    }
  };
  ...
  const sortedPurposes = [...currentItems].sort((a, b) => 
      ...
      {isFormOpen && (
        <div className="form-container">
          <div className="form-header">
            <h3>{isEditMode ? 'Edit' : 'Add New'} Purpose</h3>
          ...
          <form onSubmit={handleSubmit}>
            <div className="form-group">
              ...
              <input
                ...
                required
                autoComplete="off"
                placeholder="e.g., Registration, Admission, etc."
              />
            </div>
        ...
        {totalPages > 1 && (
          <div className="pagination">
            ...
            <div className="page-info">
              Showing {indexOfFirstItem + 1} to {Math.min(indexOfLastItem, filteredPurposes.length)} of {filteredPurposes.length} entries
            </div>
          </div>
        )}
      </div>
